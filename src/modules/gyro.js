...
(function(root, factory) {
  ...
  gyro.calibrate = function() {
    for (var i in measurements) {
      calibration[i] =
        typeof measurements[i] === "number" ? measurements[i] : 0;
    }
  };
  ...
  function quaternionToEuler(q) {
    ...
    var alpha =
      front.x == 0 && front.y == 0 ? 0 : -Math.atan2(front.x, front.y);
    var beta = Math.atan2(
      front.z,
      Math.sqrt(front.x * front.x + front.y * front.y)
    );
    var zgSide = {
      x: Math.cos(alpha),
      y: Math.sin(alpha),
